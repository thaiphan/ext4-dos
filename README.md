...
- **Cross-directory rename of a directory works in the host library but not the DOS TSR.** The host `ext4_rename_xdir` handles directories end-to-end: it rewrites the moved dir's `..` entry, adjusts both parents' `i_links_count` (-1 / +1) to reflect the lost/gained backlink, and walks the new parent's ancestor chain to refuse cycles. The DOS TSR refuses dir-rename calls; shipping the cycle-detection walker would push `_TEXT` over the 64-KiB cap, and the case is rare (`COMMAND.COM`'s `REN` is same-dir only — only direct `INT 21h AH=56h` callers reach it). Cross-directory rename of regular files works on both host and DOS.
