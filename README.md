...
- **MS-DOS 4: DOS COPY of a shorter file over an existing one** still leaves the destination at its old length with stale tail bytes. FreeDOS works (`COMMAND.COM`'s `COPY` issues `INT 21h AH=3Ch` which goes through `REM_CREATE`, and `REM_CREATE` now truncates an existing target before opening). MS-DOS 4 routes the same operation through `AX=6C00h` Extended Open → redirector `AL=0x2E`, which currently opens without truncating; the truncate intent lives in `EXTOPEN_FLAG` inside the SDA and isn't yet read by the redirector.
