...
- **TSR size sensitivity (UNRESOLVED, ~2026-05-02).** Adding even 6+ bytes of `static const` data to our DGROUP causes MS-DOS 4 to *progressively* fail the second `ext4_path_lookup` call — the first Open of `/hello.txt` succeeds, the second returns 0. FreeDOS handles any size cleanly. This is reproducible with a pure padding test (no functional changes — just `static const char pad[6]`). We bisected it to a binary-size threshold between +4 bytes (works) and +6 bytes (breaks). Linker map showed DGROUP at 29 KB / 64 KB and code at 20 KB / 64 KB — well below segment limits, so it's not a hard overflow. Likely candidates: paragraph-alignment of MCB allocation interacting with MS-DOS 4 SYSINIT memory placement, OR our resident block crossing some specific address boundary that affects the kernel's path-resolution caches. **Investigation needs the debug-build DOSBox-X to step through MS-DOS 4 internals at the moment of the second `ext4_path_lookup` failure.** Current ext4 module is at this size sweet spot (binary ~22716 bytes) — the punch list features (8.3 alias generator, journal replay, attribute additions) all need to navigate this once we figure it out.
