...
- **TSR DGROUP layout sensitivity (worked-around, root cause not fully understood).** Adding small amounts of `static const` data at the wrong place in our DGROUP causes MS-DOS 4 (only) to *progressively* fail the second `ext4_path_lookup` call — first Open of `/hello.txt` succeeds, the second returns 0. FreeDOS handles every layout cleanly. We bisected the original symptom to a +6 byte threshold and worked around it by adding several file-scope trace globals to `src/ext4/dir.c` (`ext4_path_lookup_step`, `_mode`, `_inode_rc`, `_dir_rc`) — the additional ~12 bytes shifts subsequent DGROUP statics to a configuration MS-DOS 4 doesn't trip on. **The trace globals are load-bearing — don't remove them.** Linker map at the working configuration: DGROUP 29 KB / 64 KB, code 20 KB / 64 KB. The mechanism is suspected to be MS-DOS 4 corrupting memory at a specific relative address that, before the fix, coincided with `path_lookup`'s static `inode` struct; the trace globals push that struct out of the dangerous address. Confirming this requires debug-build DOSBox-X to step the kernel's behavior at the moment of the second lookup. Until then, treat any new file-scope or function-local static introduced into the redirector as a potential layout-shift trigger and re-test on MS-DOS 4 explicitly.
