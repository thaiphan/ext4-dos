...
- **GetDiskSpace `bytes free` corruption (RESOLVED 2026-05-02).** *Symptom:* `DIR Y:` under MS-DOS 4 reported either ~2 GB or ~64 MiB free for our 64 MB fixture, vs. FreeDOS's correct ~56 MB. *Root cause:* a residual layout-sensitive corruption (a sibling of the IFS-hdr bug) — a kernel routine still ends up writing the FAT-encoded mtime through our DS at a fixed offset around 0x8AA. Whichever of our static `g_fs.sb` fields lives at that offset gets clobbered. *Fix:* snapshot the few sb fields the GetDiskSpace handler actually needs (`block_size`, `blocks_count`, `free_blocks_count`) into non-zero-initialised globals at install time. Watcom places initialised data in the `_DATA` segment (low DGROUP offsets, well below `_BSS`), out of the kernel's write target. The handler reads from the snapshot rather than `g_fs.sb`. The earlier `blocks_total > 0x18000` "don't scale spc" workaround was masking the symptom — that gate is no longer needed and has been removed.
