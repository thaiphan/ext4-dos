...
- **TSR size sensitivity (RESOLVED 2026-05-02).** *Symptom:* adding ~6 bytes of `static const` data to our DGROUP made `ext4_path_lookup("/hello.txt")` return 0 on the second Open. *Root cause:* MS-DOS 4's CDS slot has IFS fields past the 67-byte path (`curdir_type` at +0x51, `curdir_ifs_hdr` far ptr at +0x52). We were only zeroing the first 67 bytes, so the IFS-header pointer held whatever stale kernel state was there. Because MS-DOS 4 uses the same bit value (0x8000) for `curdir_isnet` and `curdir_isifs`, the kernel followed our `curdir_ifs_hdr` garbage, loaded a segment register from the dereferenced "IFS file system header", and wrote into our DGROUP — specifically a `mov [bx],dx` in the FAT time-encoder routine that landed in `g_fs.sb.block_size`. Every subsequent ext4 read then computed sectors against a corrupt block_size and INT 13h failed. The bug was layout-sensitive only in *which* of our static fields sat at the corrupted DGROUP offset. *Fix:* `hook_cds()` now zeroes all 88 bytes of the slot before populating it. Found via DOSBox-X heavy debugger BPM trap on `&g_fs.sb.block_size`; the trap fired in MS-DOS 4 kernel code at `CS:0F30` with `DS=` our DGROUP, `BX=08C4`, `DX=0x08B8` (= the DOS-packed mtime of `HELLO.TXT`).
- **GetDiskSpace `bytes free` corruption (WORKED AROUND 2026-05-02; root cause not fully traced).** *Symptom:* `DIR Y:` under MS-DOS 4 reported either ~2 GB or ~64 MiB free for our 64 MB fixture, vs. FreeDOS's correct ~56 MB. *What we know:* sibling of the IFS-hdr bug — some MS-DOS 4 kernel code path (the FAT date/time encoder at `CS=0F30 IP≈0x0220`) still ends up writing the file's encoded mtime through our DS at a fixed DGROUP offset around 0x8AA on every `DIR Y:`. Whichever of our static `g_fs.sb` fields lives at that offset gets clobbered. The exact code path that loads our segment as DS (likely a `MOV DS, [CURBUF+2]` after our redirector returns when CURBUF still holds a stale far pointer) wasn't traceable in the open-source MS-DOS 4 we have. *Workaround:* snapshot the sb fields the GetDiskSpace handler reads (`block_size`, `blocks_count`, `free_blocks_count`) into non-zero-initialised globals at install time. Watcom places initialised globals in the `_DATA` segment (low DGROUP offsets, well below `_BSS`), out of the kernel's write target. The handler reads from the snapshot rather than `g_fs.sb`. *Caveat:* the kernel write still happens — currently into unused BSS bytes. The `ext4chk /V` canary in our smoke tests asserts the live `g_fs.sb` lo16 still matches the snapshot after `DIR Y:`, so any future BSS-layout shift that lands a critical static at the kernel's write target gets caught immediately.

#### Defending against the residual kernel write

**Rule:** any field read inside an INT 2Fh handler must come from an install-time snapshot in initialised data, not from `g_fs.sb` or any other BSS variable. The `GetDiskSpace` handler in `tools/tsr.c` is the worked example — see `g_safe_block_size` and how `mount_ext4()` populates it. New handlers that need to read static FS state should follow the same pattern.

The `ext4chk /V` invocation in `scripts/run-msdos4-test.sh` and `scripts/run-freedos-test.sh` asserts `live(g_fs.sb) == snapshot` after the test sequence. If you add a new sb field to the snapshot or change BSS layout in a way that lands a previously-safe field at the kernel's write target (~DGROUP+0x8AA), the canary will trip and the smoke test will fail.
